...
export default function Home() {
  ...
  return (
    <div className="grid grid-rows-[auto_1fr_auto] h-full">
      ...
      <main className="px-page py-2 lg:py-8">
        <BotMessage content={WELCOME_MESSAGE} />
        {messages.map((message, i) => {
          return message.role === "user" ? (
            <UserMessage key={i} content={message.content} />
          ) : message.role === "assistant" ? (
            <BotMessage key={i} content={message.content} autoplay={autoplay} />
          ) : null;
        })}
        {isLoading && (
      ...
      <div
        className={
          "px-page py-6 sticky bottom-0 bg-white grid grid-cols-[1fr_auto] gap-4 items-center"
        }
      >
        {!isAudioInput && (
          <MessageInput
